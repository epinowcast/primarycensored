...
&= Q_T(t + w_P) + { 1 \over w_P} \Big[  \int_t^{t+w_P} f_T(z) z~ dz - t \Delta_{w_P}F_T(t) \Big].(\#eq:unifprim)
\end{align}
$$
Note that for any distribution with an analytically available distribution function $F_T$ equation \@ref{eq:unifprim} can be solved so long as the partial expectation $\int_t^{t+w_P} f_T(z) z~ dz$ can be reduced to an analytic expression.
...
$$
**Survival function of $S_{+}$**

By substituting equation \@ref{eq:gammapartexp} into equation \@ref{eq:unifprim} we can solve for both the survival function of $S_+$ in terms of analytically available functions:
...
$$
\begin{aligned}
f_n &= (n+1) F_T(n+1; k, \theta) + (n-1) F_T(n-1; k, \theta) - 2  n F_T(n; k, \theta) \\
