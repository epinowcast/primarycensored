...
For a faster MLE-based approach that doesn't require Stan, see the `vignette("fitting-dists-with-fitdistrplus")` vignette. For more flexible delay distribution fitting, see the [`epidist`](https://epidist.epinowcast.org) package (which uses `primarycensored` under the hood).
