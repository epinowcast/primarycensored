...
# Stan posterior PMF.  `simplex` reads np_pmf directly; `hazard` reads
# np_weights (= hazards) and converts each draw via hazards_to_pmf().
stan_pmf_draws <- function(fit, K, paramtype) {
  var <- if (paramtype == "simplex") "np_pmf" else "np_weights"
...
stan_fits <- list(
  list(fit = dirichlet_fit, paramtype = "simplex", label = "step"),
  list(fit = rw_fit,        paramtype = "hazard",  label = "hazard (RW)"),
  list(fit = re_fit,        paramtype = "hazard",  label = "hazard (RE)")
)
...
stan_df <- do.call(rbind, lapply(stan_fits, function(x) {
  d <- stan_pmf_draws(x$fit, K, x$paramtype)
  data.frame(
    bin = seq_len(K),
...
The largest errors sit in the bins between the two modes (around bins 4--6): every method overshoots bin 4 a little and undershoots bins 5--6.
This is where the truth departs most sharply from either component lognormal: the right tail of the first mode and the left tail of the second mode collide in the trough, so the convolution with the primary window pushes more mass than the truth has into bin 4 and away from bins 5--6.
The Stan posterior intervals correctly widen in this region to reflect that the data are less informative about the trough than about the peaks.
More observations, narrower primary windows, or fewer bins around the trough all shrink the disagreement.
