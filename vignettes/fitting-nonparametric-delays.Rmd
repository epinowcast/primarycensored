...
A bimodal shape like this is awkward to capture with a single parametric family and is a natural use case for the discrete step distribution.
...
The RE variant uses the same start-value helper and the same data.
...
The package model exposes the hazard random-walk parameterisation when `paramtype = "hazard"` and `hazard_model = "rw"`.
...
The RE variant uses the same model object and the same priors; the only change is `hazard_model = "re"` in `dist_options`.

```{r re-model-run, message = FALSE}
re_data <- pcd_as_stan_data(
  delay_counts,
  dist_id = 27,
  primary_id = 1,
  param_bounds = list(lower = numeric(0), upper = numeric(0)),
...
pmf_haz_rw <- extract_haz_pmf(fit_haz_rw, K, "rw")
...
stan_hazard_summary <- function(fit, K) {
  hazard_draws <- fit$draws(
    variables = paste0("np_weights[", seq_len(K), "]"),
    format    = "matrix"
  )
  pmf_draws <- t(apply(hazard_draws, 1, function(h) {
    h[K] <- 1
    hazards_to_pmf(h)
  }))
  data.frame(
    bin    = seq_len(K),
    median = apply(pmf_draws, 2, median),
    lo90   = apply(pmf_draws, 2, quantile, 0.05),
    hi90   = apply(pmf_draws, 2, quantile, 0.95)
  )
}

rw_summary <- stan_hazard_summary(rw_fit, K)
re_summary <- stan_hazard_summary(re_fit, K)
```
...
stan_df <- rbind(
  ...
  transform(re_summary, param = "hazard (RE)", method = "Stan")
)
...
Bins 3 and 4 sit between the two modes and absorb most of the disagreement between methods: under primary censoring with `pwindow = 1` or `2` the mass that the convolution puts at days 2--3 leaks into days 3--4, which weakens identifiability for those middle bins.
