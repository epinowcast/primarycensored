...
4. Round the total delays to the nearest secondary event window ($swindow$):

$$t_{rounded} = \lfloor \frac{t}{swindow} \rfloor \times swindow$$

5. Apply truncation to ensure that the delays are within the specified range $[0, D]$:
...
Here we see that without secondary censoring we get a distribution that is slightly right skewed. This is due to the primary event censoring. We also see that with secondary censoring we get a distribution that more closely matches the true log-normal distribution due to the censoring interacting. However, neither distribution matches the true distribution due to the truncation at `D` which biases both observed distributions towards shorter delays.

# Compute the primary event censored CDF for delays with `pprimarycensoreddist()`
...
ggplot(cdf_data, aes(x = x)) +
  stat_ecdf(aes(y = Empirical), geom = "step", color = "blue", size = 1) +
  ...
  labs(
    ...
    caption = paste0(
      ...
      "Red line: Theoretical CDF computed using pprimarycensoreddist()"
    )
  ) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  theme_bw() +
