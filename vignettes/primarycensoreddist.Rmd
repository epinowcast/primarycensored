...
theoretical_cdf <- pprimarycensoreddist(
  ...
  plnorm, meanlog = meanlog, sdlog = sdlog,
...
where \(F_{\text{cens}}\) is the primary event censored CDF and \(\text{swindow}\) is the secondary event window.
...
pmf_data <- data.frame(
  ...
  Empirical = empirical_pmf(seq(0, obs_time, length.out = 100))
)
