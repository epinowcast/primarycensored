...
- Added `pcd_stan_dist_id()` to allow for automatic discovery of distribution IDs for Stan models.
- Added `pcd_dist_name()` to allow for automatic discovery of distribution names for R functions as needed
for `add_name_attribute()`.
- Added documentation to suggest the use of `methods(pcens_cdf)` to find analytical solutions.
- Added new data sets `pcd_distributions` and `pcd_primary_distributions` to document the distributions
supported by `primarycensored`.
