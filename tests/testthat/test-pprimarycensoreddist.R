test_that("pprimarycensoreddist returns 0 for non-positive quantiles", {
  pwindow <- 1
  D <- 10
...
test_that("pprimarycensoreddist is monotonically increasing", {
  pwindow <- 1
  D <- 10
  q <- seq(0, D, by = 0.5)
...
test_that("pprimarycensoreddist handles finite D correctly", {
  pwindow <- 1
  D <- 10
...
test_that("pprimarycensoreddist handles custom primary distributions", {
  pwindow <- 5
  D <- 20
  cdf_uniform <- ppcens(
...
test_that("pprimarycensoreddist is consistent with dprimarycensoreddist", {
  pwindow <- 1
  D <- 10
  q <- 0:9
