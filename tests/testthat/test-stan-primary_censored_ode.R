...
test_that("Stan primary_censored_ode produces expected output", {
  t <- 4.5
  y <- 0.5
  ...
  result <- primary_censored_ode(t, y, theta, x_r, x_i)
...
test_that("Stan primary_censored_ode handles edge cases", {
  theta <- c(1.0, 0.5) # Example parameters for lognormal distribution
  x_r <- c(1.0, 1.0) # d and pwindow
  # dist_id, primary_dist_id, dist_params_len, primary_params_len
  x_i <- c(1, 1, 2, 0)

  result_near_zero <- primary_censored_ode(1e-10, 1e-10, theta, x_r, x_i)
  expect_true(is.finite(result_near_zero[1]))

  result_at_d <- primary_censored_ode(1, 0.0, theta, x_r, x_i)
  expect_true(is.finite(result_at_d[1]))
  expect_gt(result_at_d[1], result_near_zero[1])

  result_near_d <- primary_censored_ode(
...
test_that("Stan primary_censored_ode is continuous", {
  ...
  results <- vapply(
    t_values,
    function(t) primary_censored_ode(t, 0.1, theta, x_r, x_i)[1],
    numeric(1)
  )
...
test_that("Stan primary_censored_ode handles different distributions", {
  t <- 4.5
  y <- 0.5
  x_r <- c(5.0, 1.0) # d and pwindow
  ...
  result_lognormal <- primary_censored_ode(
  ...
  x_i_gamma <- c(2, 1, 2, 0)
  result_gamma <- primary_censored_ode(
  ...
  result_weibull <- primary_censored_ode(
    t, y, theta_weibull, x_r, x_i_weibull
  )
  expect_true(is.finite(result_weibull[1]))
})

test_that("Stan primary_censored_ode handles extreme parameter values", {
  t <- 4.5
  y <- 0.5
  x_r <- c(5.0, 1.0) # d and pwindow
  # dist_id, primary_dist_id, dist_params_len, primary_params_len
  ...
  result_small_scale <- primary_censored_ode(
  ...
  result_large_scale <- primary_censored_ode(
  ...
  result_small_shape <- primary_censored_ode(
  ...
  result_large_shape <- primary_censored_ode(
