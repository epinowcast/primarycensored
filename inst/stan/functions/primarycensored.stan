...
/**
  * Compute the log CDFs at integer delays for the vectorised PMF
  * @ingroup primary_censored_vectorized
  *
  * Returns the log CDF, without truncation, at each integer delay d in
  * start_idx:n. For delays with uniform primary terms and an integer
  * pwindow, q = d - pwindow is also an integer delay, so the terms at each
  * delay are computed once and used as both d and q (see
  * primarycensored_uniform_lcdf_from_terms()). This gives the same values
  * as primarycensored_lcdf() with half the delay CDF evaluations. Other
  * cases call primarycensored_lcdf() at each d.
  *
  * @param start_idx First delay to compute
  * @param n Last delay to compute
  * @param dist_id Distribution identifier
  * @param params Array of distribution parameters
  * @param pwindow Primary event window
  * @param primary_id Primary distribution identifier
  * @param primary_params Primary distribution parameters
  *
  * @return Vector of length n with the log CDF at d in entry d; entries
  * below start_idx are not set
  */
vector primarycensored_integer_lcdfs(
  data int start_idx, data int n, data int dist_id, array[] real params,
  ...
  // The internal lower bound is 0 for positive-support delays and -inf
  // otherwise, inlined so Stan's type checker treats it as data-only.
  for (d in start_idx:n) {
...
vector primarycensored_sone_lpmf_vectorized(
  ...
  // Log CDFs without truncation normalization. Start from max(1, floor(L)) to avoid computing unused CDFs when L > 0;
  // for L <= 0 (including -inf) start at 1 since F(d) = 0 for d <= 0.
  int start_idx = (!is_inf(L) && L > 0) ? max(1, to_int(floor(L))) : 1;
  vector[upper_interval] log_cdfs = primarycensored_integer_lcdfs(
